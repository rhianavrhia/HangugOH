...
- `js/storage.js` — the single source of truth for user progress (`STATE`), loaded from and saved to `localStorage` under key `hangukoh_state_v1`. All mutations (XP, streak, vocab familiarity, achievements) go through functions here (`addXP`, `markVocabSeen`, `completeLesson`, etc.) so state stays consistent and is always persisted immediately after a change.
